...
(function(jQuery)
{
  ...
  jQuery.railsAutocomplete.fn.extend({
    init: function(e) {
      ...
      function extractExtraFields(e) {
        var extra_fields = jQuery.parseJSON(jQuery(e).attr("data-extra-fields")) || {};
        for (var key in extra_fields) {
          extra_fields[key] = jQuery(key).val();
        }
        return extra_fields;
      }
      function requestData( e, term ) {
        var data = extractExtraFields(e);
        ...
        select: function( event, ui ) {
          ...
          if (e.delimiter != null) {
            ...
            if (jQuery(this).attr('data-update-elements')) {
              ...
              var update_elements = jQuery.parseJSON(jQuery(this).attr("data-update-elements"));
              for (var key in update_elements) {
                jQuery(update_elements[key]).val(data[key]).text(data[key]).trigger('change');
              }
            }
          }
